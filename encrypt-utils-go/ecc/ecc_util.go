...
func GenerateKey() {
	...
	//保存私钥
	//生成文件
	privatefile, err := os.Create("data/ecc_256.priv.pem")
	if err != nil {
		panic(err)
	}
	//x509编码
	...
	//创建文件
	publicfile, err := os.Create("data/ecc_256.pub.pem")
	//x509编码
...
//取得ECC私钥
func GetPrivateKey(path string) *ecdsa.PrivateKey {
	//读取私钥
...
//取得ECC公钥
func GetPublicKey(path string) *ecdsa.PublicKey {
	//读取公钥
	file, err := os.Open(path)
	if err != nil {
		panic(err)
...
//对消息的散列值生成数字签名
func Sign(msg []byte, path string) ([]byte, []byte) {
	//取得私钥
...
//验证数字签名
func VerifySign(msg []byte, rtext, stext []byte, path string) bool {
	//读取公钥
